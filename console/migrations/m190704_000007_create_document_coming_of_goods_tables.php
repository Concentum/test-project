    ...
    public function safeUp()
    {
            ...
            BEGIN
                IF TG_OP <> \'DELETE\' THEN 
                    ...
                    INSERT INTO goods_in_warehouse(period, product_id, warehouse_id, quantity, op, recorder_id, recorder_type) 
                        SELECT m.date_time, d.product_id, m.warehouse_id, SUM(d.quantity), 1, m.id, 1
                        FROM document_coming_of_goods m RIGHT JOIN document_coming_of_goods_product d ON m.id = d.document_id
                        WHERE m.id=\'|| NEW.id ||\'
                        GROUP BY m.date_time, d.product_id, m.warehouse_id, d.quantity, 1, m.id, 1 \';    
                END IF;
                IF TG_OP = \'UPDATE\' THEN
                    IF OLD.is_posted = true AND NEW.is_posted = true THEN
                        RAISE EXCEPTION \'You cannot change the document in posted state\';
                ...
                ELSIF TG_OP = \'DELETE\' THEN
                    IF OLD.is_posted = true THEN
                        RAISE EXCEPTION \'You cannot delete the document in posted state\';
                    ELSE 
                        RETURN OLD;
                    END IF;
                END IF;
        ...
        $this->execute('
            CREATE FUNCTION public.document_coming_of_goods_product_tf() RETURNS trigger
                ...
                AS $$DECLARE
                ROW RECORD;
            BEGIN /* При попытке удалить строку из проведённого документа генерируется исключение */
        ...
        $this->execute('
            CREATE TRIGGER document_coming_of_goods_product_trigger 
            BEFORE INSERT OR DELETE OR UPDATE ON public.document_coming_of_goods_product
            FOR EACH ROW EXECUTE PROCEDURE public.document_coming_of_goods_product_tf();
        ');

        $this->execute('
            CREATE TRIGGER document_coming_of_goods_trigger 
            BEFORE INSERT OR DELETE OR UPDATE ON public.document_coming_of_goods 
            FOR EACH ROW EXECUTE PROCEDURE public.document_coming_of_goods_tf();
        ');    
